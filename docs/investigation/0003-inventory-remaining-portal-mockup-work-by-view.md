...
- [Context](#context)
  - [Scope clarification (decided 2026-05-14 during INV-0003 review)](#scope-clarification-decided-2026-05-14-during-inv-0003-review)
- [Approach](#approach)
...
**Current portal state:** `<Topbar>` (IMPL-0004 Phase 3 + 9a + 9b) is a sticky 3-column grid (`1fr minmax(0,520px) 1fr`, `padding: var(--space-3) var(--space-6)`, `background: var(--color-bg-base)`, hairline border-bottom, `box-shadow: var(--shadow-sm)`). Left column: `<Link>` reading `rfc-site` (sans 15px 600). Centre column: a read-only `<Input>` wrapped in a `<Link to="/search">` (clicking opens `<SearchModal>`; meta-click / middle-click navigates to the standalone route), with an inline `<Kbd>⌘K</Kbd>` suffix. Right column: three inert `<span aria-disabled>` placeholders (API / MCP / Frameworks) + `<ThemeToggle>`. Cmd/Ctrl-K is bound on `document`. Below 720px, the centre search column collapses (CSS-only); the placeholder labels also hide. (9 tests.)

**Mockup expectations** (mockup §142-257 CSS + §2828-2847 / §3095-3114 body markup — the topbar is replicated identically across all 6 views):

- `.topbar { position: sticky; top: 0; z-index: 100; height: 56px; display: grid; grid-template-columns: 260px 1fr auto; padding: 0 32px; background: rgba(11, 14, 13, 0.85); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border-hairline) }`. **Fixed 56px height** (portal uses `padding: var(--space-3) var(--space-6)`, content-driven height). **Translucent dark glass** (`rgba(11,14,13,0.85)` + `backdrop-filter: blur(12px)`) — portal uses opaque `var(--color-bg-base)`.
- `.brand` is **3 elements**: `.brand-mark` (22×22 square, 1.5px accent border, accent letter "R" inside) + `.brand-name` (`rfcs`, mono 13px 500 primary) + `.brand-sub` (`/ portal`, mono 11px tertiary).
- `.topbar-search { justify-self: center; max-width: 480px; height: 34px; padding: 0 12px; background: var(--bg-raised); border: 1px solid var(--border-hairline); border-radius: var(--r-sm); cursor: pointer; transition: all 160ms var(--ease) }` — hover paints `border-color: var(--border-default); background: var(--bg-elevated)`. Contents: search-icon SVG + placeholder text `Search RFCs, authors, labels…` + right-aligned `.kbd-group` of `⌘K`.
- `.topbar-nav { display: flex; gap: 20px; font-family: var(--font-mono); font-size: 12px }`. Links are **mono 12px tertiary**; hover and `.active` paint `--fg-primary`. Order: `Directory | Frameworks | API | MCP | About`. After the nav links, `.topbar-avatar` is a 28×28 round chip with `linear-gradient(135deg, accent → accent-dim)` background + mono 11px 600 user-initials text.
- `.kbd { min-width: 18px; height: 18px; padding: 0 5px; background: var(--bg-elevated); border: 1px solid var(--border-default); border-bottom-width: 2px; border-radius: var(--r-xs); font-family: var(--font-mono); font-size: 10px }`. Note the **2px bottom border** — keycap-style affordance.

**Gap:**

- **Brand identity is wrong.** Portal: single text link `rfc-site`. Mockup: 3-element composite `[R-square-with-accent-border] rfcs / portal` where `rfcs` is primary and `/ portal` is tertiary mono. Per the RFC-only scope, `rfcs / portal` is the correct identity. Needs a `<BrandMark>` (portal-only) or inline markup in `<Topbar>`. `#topbar` `#portal-visuals` `#rfc-only-scope`
- **Translucent glass surface.** Mockup uses `background: rgba(11,14,13,0.85); backdrop-filter: blur(12px)` for a frosted-glass topbar. Portal uses opaque `--color-bg-base`. The glass effect makes the scroll-under feel intentional; without it the topbar looks "stuck on" rather than floating over content. Surface this as a `--color-bg-glass` token + a `--blur-md` (or `--backdrop-blur-md`) token in design-system, or accept it as a one-off in the portal CSS. `#topbar` `#portal-visuals` `#design-system-promote`
- **Fixed 56px height.** Portal uses content+padding which yields a slightly variable height. Mockup is rigidly 56px. The RFC sidebars sticky `top: 88px` (= 56px topbar + 32px breathing room) depend on the topbar height being known — so 56px is a load-bearing constant for the rest of the layout. `#topbar` `#portal-layout`
- **Grid template `260px 1fr auto`.** Portal uses `1fr minmax(0, 520px) 1fr` (centre-anchored search with equal flexible side columns). Mockup pins the brand column to 260px, lets the centre column flex with a `max-width: 480px` + `justify-self: center` on the inner element, then `auto`-sizes the nav. The pinned brand column matches the RFC layout's left-sidebar column width (`240px + 20px gap` ≈ 260px) — **the topbar's brand column is intentionally aligned with the metadata sidebar below**. `#topbar` `#portal-layout`
- **Search-trigger chrome.** Portal's trigger is a `<Link>` wrapping a read-only `<Input>` — this pulls in the design-system `<Input>` chrome (focus ring, generic padding). Mockup's `.topbar-search` is a chrome-less `[icon] [text] [kbd-group]` flex row with `bg: var(--bg-raised); border: 1px solid var(--border-hairline); height: 34px` and a hover swap. Either reskin via class overrides or drop the `<Input>` and inline a plain flex row. **Recommend the latter** — the trigger is decorative (the real input lives in the modal), so the abstraction adds nothing. `#topbar` `#portal-visuals`
- **Nav: inert `<span>` → real `<Link>`.** Mockup's nav links are clickable `<a>` (with `.active` paint on the current view). Portal: API / MCP / Frameworks are inert `<span aria-disabled>`. Once those views ship, swap to `<NavLink>` with active-state coloring driven by the current route. (Per the RFC-only scope, the `Directory` link target is `/`; `About` is a no-op anchor in the mockup — low priority.) `#topbar` `#portal-new-routes`
- **Nav typography.** Mockup nav is **mono 12px tertiary** with primary on hover/active; portal nav placeholders are sans 13px 500 muted. Mono is the consistent "system surface" font throughout the mockup (topbar / number-line / TOC / labels / filter toolbars). `#topbar` `#portal-visuals`
- **Avatar chip missing.** Mockup ends the nav with a 28×28 `linear-gradient(accent → accent-dim)` chip with user initials. Portal has no auth surface yet. Decision: scaffold an `<Avatar>` ds-candidate (initials sourced from user-context once auth lands) or omit until needed. **Recommend scaffolding an empty `<Avatar>` ds-candidate** so the layout reserves the space and we can promote when ready. `#topbar` `#portal-new-component`
- **Theme toggle placement.** Portal renders `<ThemeToggle>` at the end of the nav. Mockup has no theme toggle in the topbar (the mockup is dark-only). Decision: keep `<ThemeToggle>` (portal supports light/dark and the toggle is a useful escape hatch) or hide and rely on `prefers-color-scheme`. **Recommend keeping it** — light-theme tokens exist in the design system. Position-wise, slot it between the nav placeholders and the avatar chip. `#topbar` `#design-decision`
- **`<Kbd>` 2px bottom-border treatment.** Mockup's `.kbd` has `border-bottom-width: 2px` for a keycap shadow. Verify the design-system `<Kbd>` primitive matches; if not, that's a fixable CSS rule in `@donaldgifford/design-system`. `#topbar` `#design-system-verify`
- **Responsive collapse.** Portal collapses the centre search at 720px. Mockup §1530 collapses **topbar to `grid-template-columns: 1fr auto; padding: 0 20px`** (drops the search) at 800px. Behaviour-equivalent; just align the breakpoint and verify the brand-column shrink. `#topbar` `#portal-layout`
