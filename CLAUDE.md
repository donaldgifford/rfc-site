...
- Routes: `src/routes/_index.tsx` (directory `<DirectoryTable>` + Link-header pagination via `?cursor=` — IMPL-0004 Phase 7a swapped the card grid for a semantic table); `src/routes/$type.$id.tsx` (doc page with title h1, `<Badge>`, dateline, authors, `<DocumentView document={doc} />` body — Markdown rendered through the unified pipeline as of IMPL-0003 Phase 5); `src/routes/search.tsx` (IMPL-0003 Phase 7 — full-text search bound to `?q=`, results rendered with `<Snippet>`, links use the URL form per Hard rules). All three wire `RouteErrorBoundary` as their `ErrorBoundary` export.
...
src/
  ...
  components/
    ...
    ds-candidates/
      ...
      Kbd/                           ← IMPL-0004 Phase 2: keyboard-hint badge, size sm/md (5 tests)
