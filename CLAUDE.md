...
- **Phase 5 — OQ verification (closed 2026-05-19)**: **OQ-1 Shiki singleton** verified via `tests/portal/markdown/renderMarkdown.perf.test.ts` — three back-to-back renders show `cold=1701ms warm1=2ms warm2=3ms` (~590× speedup), proving Shiki's highlighter is reused across calls. **OQ-2 RR7 streaming** verified via production build + `react-router-serve` + `curl /rfc/0001`: response 47 066 bytes; `<article class="markdown-body"` opens at byte offset **4193** — well inside the 8 KB first-flush window. Dev-server response is NOT representative because Vite inlines all CSS; production never does. **Cross-doc-nav** covered in Phase 4 (8 tests). **Kitchen-sink RFC-0009** authored at `~/code/rfcs/docs/rfc/0009-rendering-pipeline-kitchen-sink-visual-fixture.md` — 13 languages, 5 mermaid types, code-block + admonition edge cases, h2-h5 depth. **Visual parity** done as a programmatic token-level diff: `tests/portal/markdown/visual-parity.test.ts` reads `~/code/design-system/rfc-portal-mockup_15.html` directly, asserts all 12 `--code-*` tokens in `src/styles/tokens.css` match the mockup verbatim, mockup's `pre {…}` references `var(--code-*)` (not raw hex — so the comparison is load-bearing), `.mermaid-diagram` container CSS uses mockup tokens per §1157-1167, language badge selector + properties match §812-823 with the `data-lang` → `data-language` adaptation. Stronger than a screenshot diff — a future mockup revision that changes a token surfaces as a test failure. **Test totals**: +1 perf + 22 visual-parity = **289 tests across 39 files** after Phase 5.
- **Phase 6 — Per-commit cache (closed 2026-05-19)**: `src/portal/markdown/renderCache.ts` — ~75 LOC wrapper around `renderMarkdown`. Module-scoped `Map<string, { html, lastAccess }>`. `MAX_ENTRIES = 256` (working-set ceiling), `ENTRY_TTL_MS = 60 * 60_000` (1h backstop). `cacheKey(doc)` → `${doc.id}@${doc.source.commit}` or `null` when commit is missing/empty (bypass to direct render). On hit: TTL-check first, then `Map.delete + Map.set` to bump to the back of insertion order (LRU semantics). On miss: render + store; if `size > MAX_ENTRIES`, pop `Map.keys().next()` (the front-most = oldest insertion = LRU victim). Test-only exports: `_clearRenderCache()`, `_renderCacheSize()`, `_RENDER_CACHE_LIMITS`. **Concurrent-call coalescing deferred** — two simultaneous misses for the same key both render (second is idempotent overwrite); a `Map<key, Promise>` for in-flight requests would solve it but the cost is small. Wired into `$type.$id.tsx` loader via a one-line import swap (`renderMarkdown` → `renderMarkdownCached`). **9 new tests** in `renderCache.test.ts` covering key shape, hit/miss/eviction/TTL/LRU-bump/null-bypass via `vi.useFakeTimers()` + `vi.setSystemTime` for TTL. **Test totals (including Phase 5 visual-parity additions)**: 258 → **289 tests across 39 files** (Phase 5 +23: 1 perf + 22 visual-parity; Phase 6 +9 cache).
