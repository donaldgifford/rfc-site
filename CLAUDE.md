...
- ds-candidates in flight (IMPL-0004): `<Button>` (Phase 1 — variants primary/secondary/ghost/icon × sizes sm/md/lg, `asChild` via `@radix-ui/react-slot`, 9 tests; consumed by `src/routes/search.tsx`). `<Input>` (Phase 2 — sizes sm/md, `prefix`/`suffix` slots, 8 tests). `<Kbd>` (Phase 2 — sizes sm/md, `--tracking-wider` typography, 5 tests). Input + Kbd land in usage in Phase 3 via the `<Topbar>`.
