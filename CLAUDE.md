...
- ds-candidates in flight (IMPL-0004): `<Button>` (Phase 1 shipped — variants primary/secondary/ghost/icon × sizes sm/md/lg, `asChild` via `@radix-ui/react-slot`, 9 tests). Currently consumed by `src/routes/search.tsx` submit form (one of the two usage sites required for Phase 6 promotion).
