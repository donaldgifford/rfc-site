...
- Tests: `getDoc` hook+MSW (IMPL-0001 Phase 3); `$type.$id` loader (200/404/500) + full-render via `createRoutesStub`; `_index` loader (cursors / Link header / query forwarding) + full-render; `<RouteErrorBoundary>` (404 + 500); `<ThemeToggle>`; IMPL-0002 fixture-loader (10) + MSW handlers (9, incl. paginated `/docs` round-trip + search-envelope contract); IMPL-0003 markdown pipeline (10) + sanitize schema (12) + `<Snippet>` (6) + `strip-docz-boilerplate` plugin (7) + `mermaid-marker` plugin (4) + `<Anchor>` component (6) + `<Pre>` / `<MermaidBlock>` (5); IMPL-0003 `/search` loader (4) + full-render (4); IMPL-0004 ds-candidates `<Tabs>` (6) + `<CodeBlock>` (5) + `<Breadcrumb>` (6); IMPL-0004 portal `<DirectoryTable>` (5) + `<DocSidebar>` (7) + `<RFCPreviewCard>` (5) + `<Topbar>` (9, incl. `?modal=1` URL state) + `<SearchModal>` (14, incl. filter pills + grouped headers + focus-trap + preview pane). **151 tests across 26 files.** (Promoted suites migrated to design-system: `<Badge>` 28 tests, `<Kbd>` 5 tests, `<Input>` 8 tests, `<Card>` 7 tests, `<Button>` 9 tests.)
...
- IMPL-0004 manual browser smoke for the Phase 8 two-column doc page + Phase 9 `<SearchModal>` (filter pills, grouped headers, ⌘K, ?modal=1 URL state, focus-trap, side preview pane). The loop's environment doesn't run a browser; jsdom-based component + render tests cover the behaviour but not the visual layout.

What's blocked externally:

- IMPL-0004 Phase 7b (`<DirectoryToolbar>` filter+sort URL state) gates on an upstream `rfc-api` contract change that adds `?filter=` / `?sort=` query params to `listDocs`. The table layout (Phase 7a) shipped without the toolbar; 7b is queued for a follow-up IMPL once the contract lands.
...
Next impl-level work: IMPL-0004 (`feat/components`) is functionally complete except for **Phase 7b**, which is blocked on an upstream `rfc-api` OpenAPI contract change adding `?filter=` / `?sort=` query params to `listDocs`. Once that lands, 7b's `<DirectoryToolbar>` + URL-state plumbing can ship as a follow-up. Beyond that, IMPL-0001 (bootstrap), IMPL-0002 (`API_MODE=msw`), and IMPL-0003 (Markdown pipeline + `/search`) all closed. The portal now renders the directory table, two-column doc page, hover preview cards, and full-featured `<SearchModal>` (filter pills + grouped results + focus-trap + preview pane + `?modal=1` URL state) end-to-end against MSW fixtures.
