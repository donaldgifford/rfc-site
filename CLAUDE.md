...
All 6 phases of [IMPL-0001](docs/impl/0001-bootstrap-portal-scaffold-per-design-0001.md) shipped. The portal SSR-renders a directory at `/` (now a semantic table with the `<DirectoryToolbar>` filter+sort URL-state stack on top — IMPL-0004 Phase 7) and a doc detail page at `/$type/$id`, both backed by the orval-generated rfc-api client through RR7 route loaders. Problem+JSON errors propagate through a shared `<RouteErrorBoundary>` that renders a not-found surface for `ErrNotFound` and a generic surface (with `request_id`) for everything else. `<Badge>` was promoted to `@donaldgifford/design-system@0.2.0` in Phase 6 and is now consumed as a published primitive. Phases 1-7 of [IMPL-0002](docs/impl/0002-wire-up-apimodemsw-local-dev-mode.md) shipped: `just dev-msw` boots the portal against a hand-curated fixture corpus with no `rfc-api` / Postgres / GitHub-webhook dependency. All 7 phases of [IMPL-0003](docs/impl/0003-wire-up-the-markdown-rendering-pipeline-per-design-0002.md) shipped on `feat/design-0002`: `Document.body` renders as proper HTML through `<DocumentView>` (unified plugin chain: remark-gfm → strip-docz-boilerplate → rehype-slug → rehype-autolink-headings → mermaid-marker → @shikijs/rehype → normalize-hast-properties → rehype-sanitize), heading IDs match `SearchResult.section_slug`, mermaid diagrams hydrate client-side via dynamic import, and a minimal `/search` route consumes `<Snippet>` against the shared MSW handlers. Live-rfc-api smoke verified end-to-end with a seeded `documents` row (commit `f8883c7` fixed a doc-page URL contract bug — see Hard rules). [IMPL-0004](docs/impl/0004-build-rfc-portal-components-per-inv-0002-inventory.md) **closed**: all 12 phase slices shipped — Phases 1-9 on `feat/components` ([rfc-site#6](https://github.com/donaldgifford/rfc-site/pull/6); full Topbar / DirectoryTable / DocSidebar / RFCPreviewCard / SearchModal stack + four ds-candidate promotions to design-system 0.4.0 via [design-system#11](https://github.com/donaldgifford/design-system/pull/11)); Phase 7b on `feat/phase-7b-directory-toolbar` ([rfc-site#8](https://github.com/donaldgifford/rfc-site/pull/8); `<DirectoryToolbar>` + URL-state filter/sort against rfc-api v0.3.0's DESIGN-0003 listDocs contract). Production build is MSW-clean except the static worker file. The wiring for every IMPL-0004 view is complete; the visual presentation of `<DirectoryToolbar>` diverges from the mockup at `design-system/rfc-portal-mockup_15.html:354-513` (text Type button + flat pill panel + native `<select>` instead of icon-only filter trigger + cascading filter menu + segmented sort toggle) and is deferred to a future IMPL — see "Visual fidelity vs the mockup" below.
...
- Tests: `getDoc` hook+MSW (IMPL-0001 Phase 3); `$type.$id` loader (200/404/500) + full-render via `createRoutesStub`; `_index` loader (8 — cursors / Link header / cursor forwarding / filter+sort forwarding / count-header capture / absent-header fallback / Link rel=next preservation / fixture-handler param round-trip) + full-render (4, incl. filter-aware empty state); `<RouteErrorBoundary>` (404 + 500); `<ThemeToggle>`; IMPL-0002 fixture-loader (10) + MSW handlers (19, incl. paginated `/docs` round-trip + search-envelope contract + Phase 7b filter/sort/count-header/Link round-trip/400-on-invalid-input); IMPL-0003 markdown pipeline (10) + sanitize schema (12) + `<Snippet>` (6) + `strip-docz-boilerplate` plugin (7) + `mermaid-marker` plugin (4) + `<Anchor>` component (6) + `<Pre>` / `<MermaidBlock>` (5); IMPL-0003 `/search` loader (4) + full-render (4); IMPL-0004 ds-candidates `<Tabs>` (6) + `<CodeBlock>` (5) + `<Breadcrumb>` (6); IMPL-0004 portal `<DirectoryTable>` (5) + `<DirectoryToolbar>` (10) + `<DocSidebar>` (7) + `<RFCPreviewCard>` (5) + `<Topbar>` (9, incl. `?modal=1` URL state) + `<SearchModal>` (14, incl. filter pills + grouped headers + focus-trap + preview pane). **177 tests across 27 files.** (Promoted suites migrated to design-system: `<Badge>` 28 tests, `<Kbd>` 5 tests, `<Input>` 8 tests, `<Card>` 7 tests, `<Button>` 9 tests.)
...
- _(none outstanding)_ Live rfc-api smoke confirmed across IMPL-0001 / 0002 / 0003 / 0004. IMPL-0004 Phase 7b browser smoke confirmed against both `just dev-msw` and a live rfc-api v0.3.0 + Postgres seed during PR review for [#8](https://github.com/donaldgifford/rfc-site/pull/8) — directory + toolbar + doc page + search modal all render end-to-end in both modes.

What's blocked externally:

- _(none — the last external gate, rfc-api's listDocs filter+sort contract, shipped in v0.3.0 and is consumed by IMPL-0004 Phase 7b.)_

What's deferred:

- **Visual fidelity vs the mockup.** Several IMPL-0004 surfaces ship functional but visually ad-hoc relative to `design-system/rfc-portal-mockup_15.html`. The most concrete divergence is `<DirectoryToolbar>` (mockup §354-513): icon-only filter trigger + two-level cascading filter menu + segmented sort toggle + uppercase-mono `RESULTS` widget vs. the current text "Type" button + flat pill panel + native `<select>` + bold-count widget. The URL state, contract integration, and test surface are decoupled from the JSX — visual rework is a future IMPL-shaped slice (likely paired with promoting `<Popover>` from ds-candidates so the cascading filter menu has a real home; per IMPL-0004 Resolved §7, Popover was deferred until a second route confirmed it was needed, and `<DirectoryToolbar>` is now that confirmation after `<SearchModal>`).
...
src/
  ...
  components/
    portal/
      ...
      DirectoryToolbar/              ← IMPL-0004 Phase 7b filter+sort URL state above DirectoryTable (10 tests)
...
tests/
  ...
  api/indexRoute.test.ts             ← IMPL-0001 Phase 4 + IMPL-0004 Phase 7b: _index loader (cursors / Link header / query forwarding / filter+sort forwarding / count headers / Link rel=next round-trip) (8 tests)
  api/indexRouteRender.test.tsx     ← IMPL-0001 Phase 4 + IMPL-0004 Phase 7b: _index full render + filter-aware empty state (4 tests)
...
Next impl-level work: **all four IMPLs (0001 / 0002 / 0003 / 0004) are closed.** The portal now renders the directory table + filter/sort toolbar + Link-header pagination, two-column doc page with hover preview cards, full-featured `<SearchModal>` (filter pills + grouped results + focus-trap + preview pane + `?modal=1` URL state), and standalone `/search` route end-to-end — against both MSW fixtures and a live rfc-api v0.3.0. The next IMPL slice is open: most likely a **visual-fidelity pass** that closes the gap between the IMPL-0004 surfaces and `design-system/rfc-portal-mockup_15.html`, with `<DirectoryToolbar>` as the most concrete starting point (mockup §354-513) and a possible `<Popover>` ds-candidate spawned to host the cascading filter menu pattern. After that: the three remaining mockup views without routes — `/api`, `/mcp`, `/frameworks` (the Topbar placeholders today) — each of which probably also needs upstream rfc-api / MCP-server work before the route can render anything substantive.
